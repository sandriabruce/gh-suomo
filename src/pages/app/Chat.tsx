...
export default function Chat() {
  ...
  async function sendVoiceNote(): Promise<string | null> {
    if (!user || !matchId || audioChunksRef.current.length === 0) return null;
    setUploading(true);
    try {
      const blob = new Blob(audioChunksRef.current, { type: mediaRecorderRef.current?.mimeType ?? "audio/webm" });
      const ext = blob.type.includes("mp4") ? "m4a" : "webm";
      const storagePath = `${user.id}/voice-notes/${matchId}/${Date.now()}.${ext}`;
      const { error: upErr } = await supabase.storage.from("profile-photos").upload(storagePath, blob, { contentType: blob.type });
      if (upErr) throw upErr;
      await sendContent(`${VOICE_PATH_PREFIX}${storagePath}`);
      setAudioUrl(null);
      audioChunksRef.current = [];
      return storagePath;
    } catch (e) {
